...
| #   | Ítem                                                                      | Estado | Notas                                                                             |
| --- | ------------------------------------------------------------------------- | ------ | --------------------------------------------------------------------------------- |
| 0.1 | Migración B2B/IVA tratamientos en producción (`20260601120000_…`)         | 🔄     | Guía: `docs/MIGRACION_B2B_IVA.md` — aplicar en Supabase (SQL Editor o `db push`)  |
| 0.2 | Actualizar `PLAN_6_BLOQUES.md` o archivarlo (sustituido por este roadmap) | ✅      | Archivado → apunta a `ROADMAP.md`                                                 |
| 0.3 | Soft-delete coherente (`activo`) en todas las tablas críticas             | ✅      | `clients`, `finance`, `treatments`, `appointments` + migración `inventory.activo` |
| 0.4 | Widget alertas stock **fijo** en dashboard (bloque 5 antiguo)             | ✅      | `WidgetAlerts` fijo sobre el grid (no en widgets arrastrables)                    |
| 0.5 | Contraste tipográfico en pantallas principales                            | ✅      | Clases `.erp-label` / `.erp-caption` + ajuste labels en `index.css`               |
| 0.6 | Tests automáticos mínimos (fiscal, facturas, checklist)                   | ✅      | `npm test` → `src/utils/fiscal.test.js` (Vitest)                                  |
| 0.7 | TypeScript gradual en `utils/` fiscal y facturación                       | 🔄     | `src/types/fiscal.d.ts` (tipos base); migración `.ts` en fases posteriores        |
...
| ---- | --------------------------------------------------- | ------ | --------------------------------------------------------------------------------- |
| 1.1  | Finanzas **modo básico** vs avanzado                | ✅      | **Mostrador** (lista + atajos) vs **Gestión completa** (3 cols, análisis, fiscal) |
| 1.2  | **Cierre de caja diario**                           | ✅      | Solo en Mostrador al pulsar **Hoy**; cuadre efectivo vs ingresos registrados      |
| 1.3  | Cliente → **Ver sus facturas**                      | ✅      | Botón en ficha → Facturas con `invoicesNavIntent`                                 |
| 1.4  | Enviar factura **PDF** (email / descarga rápida)    | ⬜      | Descarga PDF ✅; envío con adjunto automático → Fase 2                             |
| 1.5  | Alertas dashboard unificadas                        | ✅      | Citas hoy + stock (seguimientos solo por notificación navegador, no en panel)     |
| 1.6  | Recordatorio seguimiento                            | ✅      | Notificación navegador (`useSeguimientoNotifications`) si permiso concedido       |
| 1.7  | Aviso al facturar **empresa sin dirección fiscal**  | ✅      | Ficha, sesión (`SessionModal`) y `handleSession` en App                           |
| 1.8  | Presupuesto → sesión → factura (1 clic)             | ✅      | «Aceptar → sesión» en presupuestos; abre modal con cliente y precio               |
| 1.9  | Proveedores: **merge guiado** + reglas persistentes | ✅      | Modal merge por NIF; reglas en `supplierRules.js` (localStorage)                  |
| 1.10 | Directorio proveedores único                        | ✅      | `buildSupplierDirectory` aplica reglas canónicas en Finanzas/Stock                |
